...
// Набор цветовых схем (Палитры)
const DRAGON_PALETTES = [
    { name: 'TOXIC_GREEN', main: '#39ff14', dark: '#002200', eyes: '#ccff00' },
    { name: 'NEON_PINK',   main: '#ff2d95', dark: '#2d0015', eyes: '#ffffff' },
    { name: 'CYBER_BLUE',  main: '#00ccff', dark: '#001a33', eyes: '#ffffff' },
    { name: 'ALERT_RED',   main: '#ff4444', dark: '#2b0000', eyes: '#ffff00' },
    { name: 'SOLAR_GOLD',  main: '#ffd700', dark: '#2b2500', eyes: '#ffffff' },
    { name: 'VOID_PURPLE', main: '#bd00ff', dark: '#1a002b', eyes: '#00ffff' }
];

export function stopDragonProtocol() {
    if (animationId) {
...
export function startDragonProtocol() {
    stopDragonProtocol();

    // 1. РАНДОМИЗАЦИЯ
    const palette = DRAGON_PALETTES[Math.floor(Math.random() * DRAGON_PALETTES.length)];
    const N = Math.floor(Math.random() * 41) + 30; // Длина 30-70

    const wingIndices = new Set();
    const maxWings = N > 50 ? 4 : 2;
    const wingCount = Math.floor(Math.random() * maxWings) + 1;
    let currentWingPos = 8; 
    for(let k = 0; k < wingCount; k++) {
        wingIndices.add(currentWingPos);
        currentWingPos += Math.floor(Math.random() * 7) + 6;
        if (currentWingPos >= N - 10) break;
    }

    console.log(`🐉 DRAGON INCOMING. Color: ${palette.name}, Length: ${N}, Layer: BACKGROUND`);
    ...
    dragonSvg = document.createElementNS(xmlns, "svg");
    Object.assign(dragonSvg.style, {
        ...
        <defs>
            ...
            <g id="Espina" transform="matrix(1, 0, 0, 1, 0, 0)">
                <linearGradient id="LinearGradID_2" gradientUnits="userSpaceOnUse" gradientTransform="matrix(0.0229492, 0, 0, -0.0152893, 0, 0.05)" spreadMethod="pad" x1="-819.2" y1="0" x2="819.2" y2="0">
                    <stop offset="0" style="stop-color: ${palette.main}; stop-opacity: 1" />
                    <stop offset="1" style="stop-color: ${palette.dark}; stop-opacity: 1" />
                </linearGradient>
            ...
            <!-- КРЫЛЬЯ + ТЕЛО (ОБЪЕДИНЕННАЯ ГРУППА) -->
            <g id="Aletas" transform="matrix(1, 0, 0, 1, 0, 0)">
                ...
                <!-- 2. Тело (Вставлено внутрь группы крыльев, чтобы не было дырки) -->
                <path style="fill: url(#LinearGradID_2)" d="M-18.8,0Q-17.85 -5.7 -12.3 -9.6Q-11.2 -5.35 -6.5 -8.25L-6.45 -8.2L-6.2 -8.3Q1.25 -16.25 6.65 -12.4Q0.05 -12.55 0 -5.95Q2.7 -2.4 7.75 -4.1Q18 -1.45 18.8 0L-18.8 0" />
                <path style="fill: url(#LinearGradID_3)" d="M18.8,0Q18 1.45 7.75 4.1Q2.7 2.4 0 5.95Q0.05 12.55 6.65 12.4Q1.25 16.25 -6.2 8.35Q-6.35 8.25 -6.45 8.25L-6.5 8.25Q-11.2 5.35 -12.3 9.6Q-17.85 5.7 -18.8 0L18.8 0" />
            </g>
        </defs>
    ...
    switch(spawnSide) {
        case 0: spawnX = scrollX + Math.random() * winW; spawnY = scrollY - spawnOffset; break;
        case 1: spawnX = scrollX + winW + spawnOffset; spawnY = scrollY + Math.random() * winH; break;
        case 2: spawnX = scrollX + Math.random() * winW; spawnY = scrollY + winH + spawnOffset; break;
        case 3: spawnX = scrollX - spawnOffset; spawnY = scrollY + Math.random() * winH; break;
    }
    ...
    resizeHandler = () => { 
        ...
        const newDocHeight = Math.max(document.body.scrollHeight, document.documentElement.scrollHeight, document.body.offsetHeight, document.documentElement.offsetHeight, document.body.clientHeight, document.documentElement.clientHeight);
        dragonSvg.style.height = `${newDocHeight}px`;
    };
    ...
    for (let i = 0; i < N; i++) elems[i] = { use: null, x: spawnX, y: spawnY };

    for (let i = 1; i < N; i++) {
        if (i === 1) prepend("Cabeza", i);
        else if (wingIndices.has(i)) prepend("Aletas", i);
    ...
    const run = () => {
        ...
        if (rad > 60) {
            ...
            pointer.x += (centerX - pointer.x) * 0.05;
            pointer.y += (centerY - pointer.y) * 0.05;
        }
    };
